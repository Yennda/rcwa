...
number_of_layers=25;
number_of_input_file=2;

save_result_file_as='gratings/opt_sin_grating_03';
...
i_list=1:1:30;
x=(i_list)/length(i_list)*Lambda;
y=i_list;

for i=i_list
   th=2*pi*(i-1)/length(i_list);
   y1(i)=(sin(th)+1)/2*depth+Audepth;
   y2(i)=(sin(th)+1)/2*depth;
end
...
data(:,:,1)=transpose([x;y])
data(:,:,2)=transpose([x;y2])
