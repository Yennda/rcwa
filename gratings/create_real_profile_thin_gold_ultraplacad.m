...
function create_real_profile_thin_gold_ultraplacad(lambda,Lambda)
    ...
    refractive_indices=[1.33,gold, 1.33];
...
%     refractive_indices=[ 1.49 silver 1.0];
    ...
    depth=0.025;
    thickness_metal=0.045;
    % Lambda=0.300;
%     lambda=0.700;
    x=0:0.001:Lambda;
    % y=sin(2*pi./Lambda.*x).*depth/2+1.2*depth+thickness_metal;
    y=sin(2*pi./Lambda.*x).*depth/2+2*thickness_metal;
    data(:,:,1)=[x' y'];
    data(:,:,2)=[x' y'-thickness_metal];
