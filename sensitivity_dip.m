function [dip]=sensitivity_dip(wavelength,reflektivita1, reflektivita2,deltaindex)
...
%              x_forfit=x(y<halfvalue);
             ...
             fitted_polynomial = polyfit(x_forfit,y_forfit,n_poly);
             x_con=linspace(min(x_forfit),max(x_forfit),5000);
             [minimum_value2, iresonance_wavelength_2]=min(polyval(fitted_polynomial,x_con));
             resonance_wavelength_2=x_con(iresonance_wavelength_2);   
             
             
%              xx=min(wavelength):0.0005:max(wavelength);
%              yy=spline(x,y,xx);
             xx=min(x_pom):0.0005:max(x_pom);
             yy=spline(x_pom,y_pom,xx);
             kontrast=max(y_pom)-min(y_pom);
...
%                 width=fwhm(xx,spline(x_pom,y_pom,xx));
             ...
             x_fwhm=xx((xx>resonance_wavelength_2-width/2)&(xx<resonance_wavelength_2+width/2));
             y_fwhm=yy((xx>resonance_wavelength_2-width/2)&(xx<resonance_wavelength_2+width/2));
             ...
             FOM=abs_value(sensitivity./width*kontrast);%
             
             dip.sensitivity           =sensitivity;
             dip.FOM                   =FOM;
             dip.width                 =width;
             dip.sprwavelength1        =resonance_wavelength_1;
             dip.sprwavelength2        =resonance_wavelength_2;
             dip.kontrast              =kontrast;
 
             catch
             dip.sensitivity           =NaN;
             dip.FOM                   =NaN;
             dip.width                 =NaN;
             dip.sprwavelength1        =NaN;
             dip.sprwavelength2        =NaN;
             dip.kontrast              =NaN;    
             end
%              figure;
%  
