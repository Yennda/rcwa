...
number_of_orders=61; % odd number
...
plot_field_region_1=1; %1-yes, 2-no
plot_field_region_2=1; %1-yes, 2-no
plot_field_region_3=1; %1-yes, 2-no
number_of_plotted_period=1; % how many periods

size_region_1=5; % times region 2 thickness
size_region_3=5; % times region 2 thickness
...
%  2=  H_y			E_y
%  3=  E_z			H_z
field_component_operation=0; % 0-(abs)^2, 1-abs, 2-angle, 3-real, 4-imag
planar_field_component=2; %Ex, Hy, Ez in case of TM polarization;Hx, Ey, Hz in case of TE polarization
conical_field_component=4; %1-H_x, 2-H_y, 3-H_z, 4-E_x, 5-E_y, 6-E_z
...
get_local_absoption=1; % 1-yes, 2-no
...
Lambda=0.407; % grating period [um] (exception grating=10)
% Lambda=2; % grating period [um] (exception grating=10)
thickness_total=0.050; % total thickness [um] (exception grating=10)
plot_grating=1; % 1--yes, 2--no

% refractive indicies (sign for metals n_r-1i*n_i)
%---------------------------------------------------
setup_dispersion;
n1=1.33; % homogenous incident region 1
% n3=gold;%0.22-6.71*1i; % homogenous region 3
n3=1.33; % Groove indicies, disabled when grating=10
ng=n3;
nr=n3; % Ridge indices, disabled when grating=10
%---------------------------------------------------
see_diffraction_layout=1; % 1--yes, 2--no; view diffraction orders layout - planar diffraction
%---------------------------------------------------
switch grating
    ...
    case 10 % arbitrary grating from file
        ...
        dn_layer_thickness=0.5;
        ns=1.47;
        depth = 0.150;
        amplitude = 0.050;
        
        
%         create_real_profile_thin_silver(lambda,Lambda, n1, ns, dn_layer_thickness, amplitude, depth)
        input_grating_file='lyutakov_output.m';  
...
%         lambda=0.85;
%         setup_dispersion;
%         theta0=62; % incident angle [degree]70
%         n1=1.8;
%         planar_field_component=2; %Ex, Hy, Ez in case of TM polarization;Hx, Ey, Hz in case of TE polarizatio
% %         conical_field_component=2; %1-H_x, 2-H_y, 3-H_z, 4-E_x, 5-E_y, 6-E_z
%         input_grating_file='lyutakov_output_flat.m';
        
    

%         input_grating_file='grating_sinus_gold.m';
...
switch measurement
    ...
    case 1
        ...
        maximum_number_of_orders=201; % only odd number
        step=10; % step=2 
    case 2 %
%         minimum_wavelength=0.770; %um
%         maximum_wavelength=0.810; %um
        minimum_wavelength=0.65; %um
        maximum_wavelength=0.75; %um
        step=0.001;   
    case 3 % 
        minimum_thickness=0.005; %um
        maximum_thickness=0.050; %um
        step=0.002;
    case 4
        minimum_index=1; 
        maximum_index=3; 
