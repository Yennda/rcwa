...
use_dispersion=1; % 1-yes, 2-no, see file setup_dispersion.m
...
plot_field_region_1=drw_plt; %1-yes, 2-no
plot_field_region_2=drw_plt; %1-yes, 2-no
plot_field_region_3=drw_plt; %1-yes, 2-no
...
plot_grating=drw_plt; % 1--yes, 2--no

% refractive indicies (sign for metals n_r-1i*n_i)
%---------------------------------------------------
% n1=1.329; % homogenous incident region 1
% n3=0.22-6.71*1i; % homogenous region 3
ng=n1; % Groove indicies, disabled when grating=10
% n3 = 1;
% nr=n3; % Ridge indices, disabled when grating=10
%---------------------------------------------------

see_diffraction_layout=drw_plt; % 1--yes, 2--no; view diffraction orders layout - planar diffraction
%---------------------------------------------------
switch grating
    ...
    case 10 % arbitrary grating from file
        ...
        if strcmp(input_grating_file,'lyutakov_output.m')
            create_real_profile_thin_gold_SPRM(lambda,Lambda, n1, ns, dn_layer_thickness, amplitude, depth);
        end
...
% measurement=2; % measurement 1 --- check studying_order
...
switch measurement
    case 0       
    case 1
        minimum_number_of_orders=mini; % only odd number
        maximum_number_of_orders=maxi; % only odd number
%         step=2; % step=2 
    case 2 %
        minimum_wavelength=mini; %um
        maximum_wavelength=maxi; %um
%         step=0.001;   
    case 3 % 
        minimum_thickness=mini; %um
        maximum_thickness=maxi; %um
%         step=0.001;
    case 4
        minimum_index=mini; 
        maximum_index=maxi; 
...
%         change_of_index=2; % 1 --- n3=nr, 2 --- n3 is changing, 3 --- nr is changing
    case 6
        minimum_angle=mini; % theta [degree] 
        maximum_angle=maxi; % theta [degree]
%         step=0.1;
    case 7
        minimum_layer=mini; 
        maximum_layer=maxi;
%         step=5;
    case 8
        minimum_Lambda=mini; % um
        maximum_Lambda=maxi; % um
%         step=0.01;
    case 9
        minimum_duty_cycle=mini; % um
        maximum_duty_cycle=max; % um
%         step=0.00096;
    case 10
        minimum_angle_psi=mini; % [degree] 
        maximum_angle_psi=maxi; % [degree]
%         step=0.5;        
    case 11
        minimum_angle_phi=mini; % [degree]
        maximum_angle_phi=maxi; % [degree]
%         step=0.5;
