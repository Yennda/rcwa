function varargout=new_LD(lambda,material,number_of_poles,damping,imag_sign)

%function varargout = new_LD(lambda,material,model)

% new_LD(lambda,'Au',number_of_poles,tlumeni);
...
%   INPUT PARAMETERS:
% 
%       lambda   ==> wavelength (micro meters) of light excitation on material.
...
%       
%       number_of_poles ==> 1 = Drude model, >=2 = Lorentz-Drude model
%       
%       damping  ==> 1 = Gamma, 2 = Gamma=0
%   
%   REFERENCES:
%
...
% damping
if damping==1 % damping
elseif damping==2 % no damping
    Gamma=0.*Gamma;
end
...
%order
%number_of_poles
if number_of_poles>order
    error('ERROR! number_of_poles>=order')
end

epsilon_L = zeros(size(lambda));
for kk=1:number_of_poles
    if number_of_poles==1 % Drude model
        epsilon=epsilon_D;
    elseif kk>1 % Lorentz-Drude model
        epsilon_L = epsilon_L + (f(kk)*omegap^2)*...
            (((omega(kk)^2)*ones(size(lambda)) - omegalight.^2) -...
            1i*Gamma(kk)*omegalight).^(-1);
    end
